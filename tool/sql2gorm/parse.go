...
func Parse(sql string, options ...Option) (*ModelCode, error) {
	opt := parseOption(options...)

	stmtNode, err := parser.New().Parse(sql, opt.Charset, opt.Collation)
	if err != nil {
		return nil, err
	}
	tableStr := make([]string, 0, len(stmtNode))
	importPath := make(map[string]struct{})
	for _, node := range stmtNode {
...
func ParseToWrite(sql string, writer io.Writer, options ...Option) error {
	data, err := Parse(sql, options...)
	if err != nil {
		return err
	}
...
func ParseFormat(sql string, options ...Option) ([]byte, error) {
	w := strings.Builder{}
	err := ParseToWrite(sql, &w, options...)
	if err != nil {
		return nil, err
	}
...
func init() {
	var err error
	structTmpl, err = template.New(structTmplName).Parse(structTmplRaw)
	if err != nil {
		panic(err)
	}
	fileTmpl, err = template.New(fileTmplName).Parse(fileTmplRaw)
	if err != nil {
		panic(err)
	}
}
